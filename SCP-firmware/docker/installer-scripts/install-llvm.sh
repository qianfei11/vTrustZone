...
rt_version=$1
llvm_version=$(echo "$1" | awk -F'.' '{ print $1 }')
...
cmdp "Downloading the source code" wget "https://github.com/llvm/llvm-project/releases/download/llvmorg-$rt_version/compiler-rt-$rt_version.src.tar.xz" -O ${TMP_FOLDER}/rt-src.tar.xz
cmdp "Extracting the source code" tar -xf ${TMP_FOLDER}/rt-src.tar.xz -C ${TMP_FOLDER}

# We need top level common cmake files
cmdp "Downloading common LLVM cmake files" git clone --branch release/${llvm_version}.x --depth 1 --filter=blob:none --no-checkout https://github.com/llvm/llvm-project.git "${TMP_FOLDER}/llvm-project"
cmd cd "${TMP_FOLDER}/llvm-project"
cmd git sparse-checkout set --cone cmake/Modules
cmd git checkout

cmd mkdir -p "${TMP_FOLDER}/compiler-rt-build"
cmd cd "${TMP_FOLDER}/compiler-rt-build"
...
for ARCH in armv7m armv7em armv8m.main armv8.1m.main; do
    cmdp "Preparing Compiler-RT builtins for target ${ARCH}" cmake \
        ../compiler-rt-$rt_version.src \
        -G Ninja \
        ...
        -DLLVM_COMMON_CMAKE_UTILS="${TMP_FOLDER}/llvm-project/cmake" \
    ...
    cmdp "Installing the builtins" \
        cp ./lib/baremetal/libclang_rt.builtins-${ARCH}.a \
